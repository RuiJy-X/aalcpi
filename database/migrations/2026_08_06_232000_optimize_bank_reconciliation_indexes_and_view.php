...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('internal_disbursements', function (Blueprint $table) {
            $table->index(['bank_statement_id', 'date_issued'], 'idx_internal_bank_date');
        });

        Schema::table('bank_statements', function (Blueprint $table) {
            $table->index(['id', 'tdate'], 'idx_bank_id_tdate');
        });
    ...
    {
        Schema::table('internal_disbursements', function (Blueprint $table) {
            $table->dropIndex('idx_internal_bank_date');
        });

        Schema::table('bank_statements', function (Blueprint $table) {
            $table->dropIndex('idx_bank_id_tdate');
        });
    }
    ...
    private function createReconciliationWorkspaceView(): void
    {
        ...
        DB::statement("
            ...
            LEFT JOIN import_jobs AS bank_job ON bank_job.id = bank_statements.import_job_id
            WHERE bank_statements.id NOT IN (
                SELECT bank_statement_id FROM internal_disbursements WHERE bank_statement_id IS NOT NULL
            )
        ");
    }
};
